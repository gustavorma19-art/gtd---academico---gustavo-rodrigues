...
export default function Home(){
  ...
  return <main className="shell">
    <aside><div className="brand"><b>🎓 GTD Acadêmico</b><small>Sistema de Organização</small></div><nav>{nav.map(([id,i,n])=><button key={id} className={active===id?'active':''} onClick={()=>setActive(id)}>{i}<span>{n}</span></button>)}</nav><footer>{session.user.email}<br/><button className="logout" onClick={signOut}>Sair</button><br/><small>© Gustavo Rodrigues · GTD de David Allen</small></footer></aside>
    <section className="content">
      <header><div><span>GTD ACADÊMICO</span><h1>{nav.find(n=>n[0]===active)?.[2]}</h1></div><div className="headerActions"><button className="ghost" onClick={()=>setActive('dashboard')}>Visão geral</button>{canAdd&&<button disabled={loading} onClick={()=>setModal(active==='dashboard'?'tasks':active==='calendar'?'assessments':active)}>＋ Adicionar</button>}</div></header>
